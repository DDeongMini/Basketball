...
public:
    ...
    bool Invade_Foot_to_Green(cv::Mat input_frame, cv::Mat ori_frame, const std::vector<cv::Point>& Red_contour, const std::vector<cv::Point>& Green_contour);
    float EllipseAngle(const cv::Mat& input_frame, const std::vector<cv::Point>& contour);
    ...
    bool Get_contain_green_area() const;
    ...
    void Set_contain_green_area(bool contain_green_area);
...
private:
    ...
    bool contain_green_area_ = false;
    ...
    mutable std::mutex mtx_contain_green_area;
};
