...
bool Img_proc::Invade_Foot_to_Green(cv::Mat input_frame, cv::Mat ori_frame, const std::vector<cv::Point>& Red_contour, const std::vector<cv::Point>& Green_contour)
{
    bool invade_green = false;

    cv::Point minYPtRed = *std::min_element(Red_contour.begin(), Red_contour.end(),
                                             [](const cv::Point& a, const cv::Point& b) {
                                                 return a.y < b.y;
                                             });

    cv::Point maxYPtGreen = *std::max_element(Green_contour.begin(), Green_contour.end(),
                                              [](const cv::Point& a, const cv::Point& b) {
                                                  return a.y < b.y;
                                              });

    cv::circle(ori_frame, minYPtRed, 5, red_color, -1);
    cv::circle(ori_frame, maxYPtGreen, 5, red_color, -1);

    cv::rectangle(ori_frame, cv::Point(200, 200), cv::Point(440, 480), cv::Scalar(0, 0, 255), 2);

    if(minYPtRed.y - maxYPtGreen.y < 0)
    {
        invade_green = true;
        putText(ori_frame, "Invade : True", cv::Point(10,100), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);

    }
    else
    {
        invade_green = false;
        putText(ori_frame, "Invade : False", cv::Point(10,100), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);
    }

    return invade_green;
}

float Img_proc::EllipseAngle(const cv::Mat& input_frame, const std::vector<cv::Point>& contour)
{
    if (contour.size() < 5)
    {
...
void Img_proc::webcam_thread()
{
    ...
    while (ros::ok())
    {
        ...
        cv::Mat roi_frame = frame.clone();

        cv::Mat Roi_Invade_Foot_Green = ROI_Rectangle(roi_frame, 200, 480, 200, 440);

        std::vector<cv::Point>Green_contour = std::get<2>(Green_Exis);
        std::vector<cv::Point>Red_contour = std::get<2>(Foot_Exis);
        ...
        if(is_Green)
        {
            ...
            Distance_foot_to_green = MinDistanceBetweenContours(std::get<0>(Green_Exis), Green_contour, Red_contour);
            angle_Green = EllipseAngle(std::get<0>(Green_Exis), Green_contour);

            this->Set_contain_adjust_to_foot(Distance_foot_to_green);
            this->Set_adjust_angle(angle_Green);

            bool Invade_Foot_Green = Invade_Foot_to_Green(Roi_Invade_Foot_Green, std::get<0>(Green_Exis), Red_contour, Green_contour);
        }
        else
        {
...
bool Img_proc::Get_contain_green_area() const
{
    std::lock_guard<std::mutex> lock(mtx_contain_green_area);
    return this->contain_green_area_;
}
...
void Img_proc::Set_contain_green_area(bool contain_green_area)
{
    std::lock_guard<std::mutex> lock(mtx_contain_green_area);
    this->contain_green_area_ = contain_green_area;
}
